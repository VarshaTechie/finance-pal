...
const getSummary = async (req, res, next) => {
    try {
        ...
        // Determine Effective Monthly Income for the period
        let effectiveMonthlyIncome = user.monthlyIncome;
        if (currentPeriodStart) {
            const incomeMonth = new Date(currentPeriodStart.getFullYear(), currentPeriodStart.getMonth(), 1);
            const incomeDoc = await Income.findOne({ userId, month: incomeMonth });
            if (incomeDoc) {
                effectiveMonthlyIncome = incomeDoc.amount;
            }
        }
        ...
        if (prevDateFilter) {
            ...
            const prevIncomeMonth = new Date(prevStart.getFullYear(), prevStart.getMonth(), 1);
            const prevIncomeDoc = await Income.findOne({ userId, month: prevIncomeMonth });
            if (prevIncomeDoc) {
                prevMonthlyIncome = prevIncomeDoc.amount;
            } else {
                // If specific prev income not found, we assume it was same as base user income 
                // (or could assume same as current effective if user income is not reliable history)
